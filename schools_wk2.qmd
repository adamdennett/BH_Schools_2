...
-   **Consequently, no change to current catchment boundaries is required in 2026 while still hitting Council PAN reduction targets.**
...
In order that we are comparing like-with-like, let's only look at state-funded 11-16 schools in England and Wales. The histogram below shows the distribution of the number of pupils in these schools. The red and cyan lines show the number of pupils at Longhill and PACA respectively.
...
ggplot(., aes(x = number_of_pupils)) +
  ...
  annotate("text", x = 876, y = Inf, label = "PACA", vjust = 18.5, color = "lightgreen") +
  theme_minimal()
