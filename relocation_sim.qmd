---
title: "What if we relocate Longhill?"
author: "Adam Dennett"
...
coords <- st_coordinates(brighton_sec_schools_sml)

brighton_sec_schools_sml1 <- brighton_sec_schools_sml
brighton_sec_schools_sml1$x <- coords[, 1] 
brighton_sec_schools_sml1$y <- coords[, 2]

#write_csv(brighton_sec_schools_sml1,here("data", "brighton_sec_schools_sml1.csv"))
...
bh_sec_sch1 <- brighton_sec_schools_relocate %>% 
...
map <- tm_shape(BTN_LSOA_Point_Proj) +
  ...
  tm_borders() +
  tm_fill(alpha = 0) +
  tm_shape(brighton_sec_schools_relocate) +
  tm_dots()
...
optionX_stats <- brighton_sec_schools_relocate %>%
...
optionA_stats <- brighton_sec_schools_relocate %>%
...
optionB_stats <- brighton_sec_schools_relocate %>%
...
optionC_stats <- brighton_sec_schools_relocate %>%
...
brighton_sec_schools_relocate$urn1 <- as.character(brighton_sec_schools_relocate$urn)
...
btn_cij <- btn_cij %>% 
  left_join(brighton_sec_schools_relocate %>% select("urn1", "number_of_pupils", "pan2024", "fsm"), by = c("dest" = "urn1"))
...
#write_csv(btn_cij, here("data", "btn_cij.csv"))

# Display the new dataframe
#print(btn_sch_gravity1)

brighton_sec_schools_relocate$prodsim1 <- btn_sch_gravity1$prodSIM1[match(brighton_sec_schools_relocate$urn, btn_sch_gravity1$dest)]

temp <- brighton_sec_schools_relocate %>%
...
write_csv(simple_ttm3, here("data", "simple_ttm_relocate.csv"))
simple_ttm3 <- read_csv(here("data", "simple_ttm_relocate.csv"))
