...
The council has announced that Whitehawk will be removed from Longhill High's catchment from a future admissions round. The stated logic — articulated informally by some parents and by parts of the council — is that the Whitehawk ward, the most deprived ward in the city, disproportionately shapes how Longhill is perceived by other families in the catchment. Middle-class parents in Woodingdean, Rottingdean, Ovingdean and Saltdean are, on this theory, being driven by reputation effects to send their children elsewhere — to Lewes Priory over the county line, or to Cardinal Newman and Kings across the city — at a higher rate than any other B&H catchment produces. Remove Whitehawk from the nominal catchment, the argument goes, the perceived demographic mix shifts, parent confidence recovers, and uptake rates follow.
...
On balance, the data as we have it points to the policy being **unlikely to reverse Longhill's decline**, while carrying a real risk of accelerating it. Four strands of evidence, in rough order of importance:
...
3.  **The next-biggest leakage is geographic, not reputational.** The **38 Longhill-catchment offers to Priory School in Lewes** in 2024 almost certainly come from Saltdean families, for whom Priory is the closest secondary school by road. That flow is about proximity, not about who else lives in the catchment, and would continue regardless of the Whitehawk boundary change. Similarly, BACA and Dorothy Stringer pick up pupils from the western fringe of Longhill's existing catchment on proximity grounds. None of these flows are reputation-driven in the sense the policy assumes.
4.  **Removing Whitehawk may reduce the catchment's own retained intake.** This is speculative but structurally worth noting. Whitehawk has the lowest rates of private-sector take-up in the catchment (deprivation correlates strongly with state-sector uptake), the lowest rates of car ownership (which restricts the choice set to schools reachable on foot or by bus), and is further from Cardinal Newman, Kings and Priory than Woodingdean or Saltdean are. On any reasonable prior, Whitehawk is the part of the Longhill catchment most likely to have *already been* sending its children to Longhill. If that prior is right, the direct mechanical effect of removing it is to **strip out exactly the part of the catchment that was propping up Longhill's numbers**, leaving a residual catchment that is more affluent on paper but that has a longer track record of *not* choosing Longhill. The 2025 figure of 97 offers and the 2026 figure of 81 offers are the post-leakage numbers *with* Whitehawk still in the catchment; the post-leakage numbers without Whitehawk plausibly land lower, not higher.

There is a narrower version of the policy argument that the data cannot refute: it is possible that Whitehawk's inclusion in the catchment is *such* a strong signalling effect that removing it produces a behavioural change among the Woodingdean / Rottingdean / Ovingdean / Saltdean wards large enough to more than offset the direct intake loss. This would require, in round numbers, something like a 15–25 percentage-point uplift in uptake among the remaining catchment wards within a few years. There is no precedent in the B&H data (or, to our knowledge, in published evidence from other English local authorities using catchment changes to manage reputation) that movements of that size are achievable from a boundary label alone. Reputation recovers when the school's offer changes — outcomes, Ofsted grades, leadership, curriculum — not when the catchment boundary is redrawn around it.

**The honest summary**, based on the data we have: the policy is at best **a marginal lever on the behavioural side of a problem that is mostly demographic and mostly faith-school-driven**, and at worst **actively subtracts from Longhill's intake** by removing the part of the catchment most likely to have been using the school. The council's own Oct-25 forecast, which already assumes a rising leakage rate, still undershoots observed reality by 30–50 pupils a year at Longhill. A catchment redraw does not, by itself, make any of those 30–50 pupils reappear. Reversing Longhill's decline, if it is reversible at all, would need a package of changes acting on the *offer* (the school's outcomes and character) alongside the catchment redraw — and in the meantime the honest expected trajectory is continued Year 7 offers in the 80–110 range, against a PAN that is substantially larger, for the remainder of the decade.

That is not a reason to oppose the Whitehawk change specifically — the equity case for not using a single deprived ward as a branding device for a school it doesn't actually attend in great numbers is strong regardless of the population dynamics. But it is a reason to be cautious about presenting the change as a *recovery plan* for Longhill. The data does not support the hope that the label will do the work on its own.
