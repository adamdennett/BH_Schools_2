...
The council's forecast uses the Oct 2024 **school census** — it knows exactly how many children currently in each year group of primary school live in each catchment, and ages them forward by year. We don't have the census, but we do have something closely related: **primary offers by catchment** from this document, going back to 2014. Each primary cohort admitted to Reception in year *Y* will be applying for Y7 in year *Y + 7*. So we can build an alternative projection by:

1. Computing, for each of the six catchments + Religious schools, the ratio **Secondary Y7 offers(Y + 7) ÷ Primary Reception offers(Y)** across every pair of years where we have both values (Reception 2014 → Y7 2021, all the way through Reception 2019 → Y7 2026 — six overlapping cohorts).
2. Averaging those ratios to get a **retention ratio** per catchment. This number absorbs in-state-sector leakage, movement into and out of the catchment, and the pull of Kings / Cardinal Newman at age 11 — it's whatever the data shows actually happens between Reception and Y7 in each catchment.
3. Applying that retention ratio to the primary offers made in 2020–2024 to project Y7 offers in **2027–2031** (the years the council also forecasts but we have no actual data for yet).

This method is close in spirit to the council's approach but differs in three respects: (a) we use primary-offer data rather than the census; (b) the retention ratio is fitted from actual Y7 outcomes rather than decomposed into explicit leakage / CN-Kings components; and (c) because actual Y7 offers are **PAN-capped**, catchments that routinely fill to PAN (Varndean / Dorothy Stringer, the Religious schools) will have their retention ratios biased toward the PAN ceiling — so our projection for those catchments effectively projects "more of the same PAN-constrained supply" rather than latent demand.
