...
p_apps_std <- ggplot(sec_std,
  ...
  geom_point() +
  scale_x_continuous(breaks = seq(2013, 2026, 1)) +
  labs(title = "Applications per Preference Slot, Secondary 2013–2026",
       x = "Year",
...
# Used by the by-catchment cohort chart below and the proportional-circle
# maps further down. Secondary faith schools (Kings, Cardinal Newman) are
# overridden to "Religious schools" because they admit city-wide and sit
# outside the geographic framework. Catholic primaries are also grouped as
# "Religious schools" so they are compared against Cardinal Newman-style
# secondary demand rather than against their geographic catchment.
library(sf)
...
ggplotly(p_cohort_catch, tooltip = "text") %>%
  plotly::layout(legend = list(orientation = "h", x = 0, y = 1.05))
```
...
# Council Forecast (BHCC Appendix 6, Oct 2024) vs Actual Offers

The table below is transcribed from [Appendix 6 — Secondary School Pupil Forecast](https://democracy.brighton-hove.gov.uk/documents/s211108/Appendix%206%20-%20Secondary%20School%20Pupil%20Forecast.pdf) of the Brighton & Hove City Council pupil-forecast paper (data source: **October 2024 school census**). It projects Year 7 demand for each catchment at the seven secondary-entry years from **2025 to 2031** — i.e. the children who were in Year 6 down to Reception at the time of the Oct 2024 census.

The council's method (summarised from the PDF):

1. Start with the raw number of pupils living in each catchment in the relevant year group of the Oct 2024 census.
2. Reduce by a catchment-specific leakage percentage (`2.01%`–`22.61%`) for pupils expected to move out of the state-maintained sector (independents, home-ed, out-of-city).
...
bhcc_surplus <- tibble::tribble(
  ~CatchmentGroup,                 ~Y2025, ~Y2026, ~Y2027, ~Y2028, ~Y2029, ~Y2030, ~Y2031,
  "PACA",                             14L,    -1L,    35L,    29L,    38L,     6L,    70L,
  "Hove Park / Blatchington Mill",    -2L,    76L,    67L,   100L,    97L,   159L,   145L,
  "Varndean / Dorothy Stringer",      49L,     6L,    38L,    42L,    70L,    73L,    91L,
  "Longhill",                         42L,    35L,    44L,    34L,    51L,    59L,    56L,
  "BACA",                             46L,    51L,    42L,    37L,    68L,    85L,    55L,
  "Patcham",                          27L,    20L,    13L,    24L,    31L,    62L,    46L
)

# Add the totals row for forecast demand (from the PDF).
bhcc_forecast_with_total <- bhcc_forecast %>%
  dplyr::bind_rows(
    tibble::tibble(
      CatchmentGroup = "Total (catchment schools)",
      PAN    = 1975L,
      Y2025  = 1800L, Y2026 = 1787L, Y2027 = 1737L, Y2028 = 1709L,
      Y2029  = 1620L, Y2030 = 1531L, Y2031 = 1512L
    ),
    tibble::tibble(
      CatchmentGroup = "Total (all schools, incl. Kings + CN)",
      PAN    = 2500L,
      Y2025  = 2297L, Y2026 = 2284L, Y2027 = 2234L, Y2028 = 2206L,
      Y2029  = 2117L, Y2030 = 2028L, Y2031 = 2009L
    )
  )
...
knitr::kable(bhcc_forecast_with_total,
             ...
             align = "lrrrrrrrr",
             caption = "Forecast Y7 demand (adjusted for Cardinal Newman & Kings), BHCC Oct-24 census projection")
```
...
knitr::kable(bhcc_surplus,
             col.names = c("Catchment",
                           "2025", "2026", "2027", "2028", "2029", "2030", "2031"),
             align = "lrrrrrrr",
             caption = "Surplus (+) or shortfall (–) of places: PAN minus forecast adjusted demand")
```

Positive values = spare capacity; negative values = demand exceeds PAN. Across the city the council's forecast shows a rising surplus — from **8 %** of all secondary places in 2025 to **20 %** in 2031 — driven mainly by falling birth cohorts now working through the system.

## How Did the 2025 and 2026 Forecasts Compare With What Actually Happened?

The 2025 and 2026 columns in the forecast are the two years for which we already have admissions data in this document. The table below sets the council's **projected demand** (from Appendix 6) alongside the **actual number of offers** we see for each catchment's schools.
...
- The Kings + Cardinal Newman totals in the PDF aren't broken out as a catchment row — instead each geographic catchment's projection is reduced each year by a *per-year* estimate of how many of its resident pupils will go to Cardinal Newman (total across all catchments **330 / yr**) and Kings (**167 / yr**). Summed, that's roughly **497 / yr** of projected faith-school intake, vs a combined PAN of **540** (CN 360 + Kings 180) — so the PDF implicitly assumes Kings and CN run essentially full, with any spare places filled from outside the six geographic catchments (e.g. out-of-city faith applicants).
...
compare_2526 <- bhcc_forecast %>%
  dplyr::select(CatchmentGroup, PAN,
                Forecast_2025 = Y2025, Forecast_2026 = Y2026) %>%
  dplyr::left_join(actual_offers_2526, by = "CatchmentGroup") %>%
  dplyr::mutate(
    Diff_2025 = Actual_2025 - Forecast_2025,
    Diff_2026 = Actual_2026 - Forecast_2026
  ) %>%
  dplyr::select(CatchmentGroup, PAN,
                Forecast_2025, Actual_2025, Diff_2025,
                Forecast_2026, Actual_2026, Diff_2026)

knitr::kable(compare_2526,
             col.names = c("Catchment", "PAN",
                           "Forecast 2025", "Actual 2025", "Δ 2025",
                           "Forecast 2026", "Actual 2026", "Δ 2026"),
             align  = "lrrrrrrr",
             format.args = list(big.mark = ","),
             caption = "BHCC Appendix 6 forecast vs actual Y7 offers, 2025 and 2026")
```

```{r}
# Separate block for the religious schools since they're not one of the
# six geographic catchments. PDF per-year implied demand = sum across all
# catchments of "Estimated number going to CN / Kings".
relig_actual <- secondary_all %>%
  ...
  dplyr::mutate(`CN + Kings total` = `Cardinal Newman` + `Kings School`,
                `PDF implied demand (per yr)` = 330L + 167L)

knitr::kable(relig_actual,
             align  = "lrrrr",
             format.args = list(big.mark = ","),
             caption = "Religious schools: actual Y7 offers vs the PDF's implied per-year demand (CN + Kings, summed across catchments)")
```

**Headline read for 2025 and 2026:**

- **Varndean / Dorothy Stringer** — the forecast under-estimated demand on both years. Forecast **581** in 2025 and **624** in 2026, but actual offers hit the PAN cap of **630** in both years (Δ +49 and +6). These two schools stay full regardless of the council's in-catchment demand model, because out-of-catchment demand tops them up. That's consistent with their long-standing over-subscription.
- **Longhill** — the forecast *over-estimated* demand dramatically. Forecast **168 / 175**, actual offers only **97 / 81** (Δ –71 and –94). Offers are roughly **half** what the council modelled, and well below PAN 210. This is the single biggest forecast-vs-actual gap in the table.
- **BACA** — 2025 actual (**89**) fell well below forecast (**134**), but 2026 flipped with actuals **146** against a forecast of **129** (Δ +17). The 2026 uptick mirrors the PAN increase from 180 to 180 and the modest city-wide 2026 rebound visible in the earlier charts — it will be worth watching whether this continues in 2027.
- **PACA** — under-filled in both years relative to both forecast and PAN. 2025 offers **190** (forecast **206**, PAN 220), 2026 offers **183** (forecast **221**, PAN 220). So the council's model has been slightly optimistic for PACA — real-world demand was a bit softer than Oct-24 census data alone would suggest.
- **Hove Park / Blatchington Mill** — 2025 tracked the forecast closely: offers **501** vs forecast **512** (Δ –11), both close to the combined PAN of **510**. 2026 is the striking case — the PDF projected a sharp drop in catchment demand to **434** (a shortfall position), but actual offers came in at **466**, i.e. **32 *above*** forecast (and still under PAN). Out-of-catchment demand appears to be filling some of the gap the council's demand-side model predicted.
- **Patcham** — forecast and actual essentially agree in both years (offers 202 / 204 against forecast 198 / 205), small Δ of +4 and –1. The most accurately forecast catchment in the table.
- **Religious schools (Kings + CN)** — both schools are at PAN in both years (CN 360 + Kings 180 = **540**), vs a PDF implied per-year demand of **497** summed across the six catchments. The **~43 / yr** gap is the outside-catchment faith intake (out-of-city Catholic applicants, siblings, etc.) that the PDF's catchment-side model doesn't see — exactly the flow that makes these schools city-wide admissions problems rather than local ones.

**Bottom line** — Varndean / Dorothy Stringer, Patcham and the religious schools tracked the forecast closely or ran *full* regardless of forecast, so the council's demand model is a reasonable guide to supply-constrained catchments. The PDF over-estimates demand at **Longhill, PACA and BACA** (where schools run below PAN anyway, so forecast error doesn't translate to unmet demand), and under-estimates it at **Varndean / Stringer, Hove Park / Blatch (2026) and the religious schools** (where out-of-catchment demand fills any forecast gap). The practical implication: places-surplus forecasts for the under-filled catchments are probably *optimistic* — actual under-fill at Longhill and BACA is worse than the PDF's "adjusted demand vs PAN" surplus row would suggest.

At the city level the council projects aggregate demand of **2,297** (2025) and **2,284** (2026). Summing actual 2025 + 2026 offers across all nine secondary schools from our data provides a direct check — any gap reflects a mix of (a) leakage rates the council has already baked in, (b) out-of-city and in-year late applicants, and (c) schools running below PAN where catchment demand fell short.
...
```{r}
# --- Project 2027..2031 secondary offers from primary 2020..2024 ---
projection <- primary_by_catch %>%
  dplyr::filter(Year >= 2020L, Year <= 2024L) %>%
  dplyr::rename(Primary_year = Year) %>%
...
# Pull our projection into a wide table alongside the council's forecast
...
# Council forecast (reshaped) — use only the six geographic catchments here;
# the PDF doesn't publish a Religious-schools demand row.
council_long <- bhcc_forecast %>%
  dplyr::select(CatchmentGroup, PAN,
                `Council_2025` = Y2025, `Council_2026` = Y2026,
                `Council_2027` = Y2027, `Council_2028` = Y2028,
                `Council_2029` = Y2029, `Council_2030` = Y2030,
                `Council_2031` = Y2031)
...
knitr::kable(
  ...
  align = "lrrrrrrrrrrr",
  format.args = list(big.mark = ","),
  caption = "Council's Appendix 6 forecast vs our cohort-aged projection, Y7 2027-2031"
)
```
...
series_council <- bhcc_forecast %>%
  ...
  dplyr::mutate(Year = as.integer(sub("^Y", "", YearStr)),
                Series = "Council forecast (Appendix 6)") %>%
  dplyr::select(CatchmentGroup, Year, Offers, Series)

series_all <- dplyr::bind_rows(series_actual, series_proj, series_council) %>%
...
p_proj <- ggplot(series_all,
  ...
  geom_line(linewidth = 0.8) +
  geom_point(size = 1.4) +
  scale_colour_manual(values = c("Actual offers"                   = "#1f78b4",
                                 "Our projection (cohort-aged)"    = "#33a02c",
                                 "Council forecast (Appendix 6)"   = "#e31a1c")) +
  facet_wrap(~ CatchmentGroup, ncol = 3, scales = "free_y") +
  labs(title = "Y7 offers: actuals, our cohort-aged projection, and the council forecast",
       subtitle = "Dotted grey lines = catchment PAN; projection uses primary offers(Y) \u00d7 retention ratio as secondary offers(Y+7)",
       x = NULL, y = "Y7 offers",
       colour = NULL) +
  theme_minimal(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
- Blue = actual Y7 offers we have in this document (2013–2026).
- Green = our projection from primary cohort aging (2027–2031).
- Red = the council's Appendix 6 demand forecast (published 2025–2031).
- Dotted grey line = catchment PAN (not plotted for the Religious schools panel, which has no single PAN).

**Catchment-by-catchment read (2027–2031):**

- **Longhill** — the biggest gap in the chart. Our projection is **96–112 offers/yr**, the council's is **151–176**. At a retention ratio of **0.50**, only half of a Longhill primary-catchment cohort has historically ended up with a Longhill Y7 offer. Projected forward, that implies Longhill runs at **under 50 % of PAN (210)** every year through 2031 — substantially below even the council's already-cautious forecast. If our projection is right, the council's "surplus of places" row materially understates how empty Longhill will be.
- **BACA** — our projection (**74–108 offers/yr**) is 20–30 % below the council's (**95–143**). BACA's retention ratio is **1.27** (it pulls in more Y7 pupils than its own primaries produce), but its primary cohort base is small enough that the projection still comes out well below PAN (180) in every year. The 2030 dip to 74 reflects an unusually small 2023 Reception cohort in the BACA primary catchment.
- **PACA** — our projection (**147–159**) sits consistently below the council's (**150–214**), by about **10–25 offers/yr**. PACA's primary base times the fitted retention ratio (**0.77**) gives a flatter, lower trajectory than the council's rising-then-falling demand profile. Both imply PACA running well below PAN (220) throughout.
- **Hove Park / Blatchington Mill** — this is the one catchment where our projection is **above** the council's. We project **414–471** offers/yr; the council forecasts **351–443** demand. The two schools have historically absorbed more pupils than the Hove Park / Blatchington Mill primary catchment delivers (retention ratio **0.65** — i.e. lots of their primary-age pupils leak to CN / Kings / independents — but out-of-catchment secondary demand *more than* compensates). Projected forward, we expect Y7 offers to stay close to current ~500 levels and drift to ~410 by 2031; the council thinks it falls further, to ~365.
- **Varndean / Dorothy Stringer** — our projection (**521–595**) is just below the council's (**539–592**). Both trend downward. Neither reaches PAN (630) on central assumptions — a meaningful shift away from the past decade's over-subscribed pattern, driven by shrinking primary cohorts at both ends of the funnel. If either our retention ratio is a little conservative or out-of-catchment demand holds up, the schools will continue filling to PAN; but for the first time in this dataset, neither model is *predicting* that outcome.
- **Patcham** — our projection (**161–196**) is close to the council's (**163–212**), with the two lines essentially tracking each other at about 88–95 % of PAN (225). Patcham is the catchment where the two methods agree best.
- **Religious schools** — our projection drifts downward from the current **540** offers (= PAN) to roughly **500** by 2030, then bounces to **543** in 2031 as a slightly larger 2024 Catholic-primary cohort arrives. The council doesn't publish a matching demand line for this group, so no direct comparison is possible. Note the retention ratio of **2.57** is inflated because Catholic primaries are small compared with Kings + CN combined — it mechanically includes every non-Catholic-primary pupil joining Kings at Y7. What matters here is the *trend* (downward through 2030), not the absolute value.

**How to read the disagreement.** Where the green (ours) and red (council) lines disagree, it's not that one is "wrong" — they're measuring slightly different things. The council models projected demand from catchment residents, built up from the Oct-24 school census; our projection multiplies primary offers by an empirically fitted secondary/primary ratio, which absorbs everything that happens to a cohort between ages 4 and 11 (in-state-sector leakage, cross-catchment flows, and PAN-capping) in a single per-catchment number. Where the two methods agree (Patcham, Varndean / Stringer, Hove Park / Blatchington Mill broadly) we can have more confidence in the forecast direction. Where they disagree sharply (Longhill, PACA, BACA), the mechanisms baked into each method are pulling in different directions — and the outturn will depend on whether the council's demand-side assumptions or our empirical flow ratios are closer to what actually happens after 2026.
...
# Council's "Total for all Schools" row from Appendix 6 (incl. Kings + CN)
city_council <- tibble::tibble(
  Year    = 2025:2031,
  Offers  = c(2297L, 2284L, 2234L, 2206L, 2117L, 2028L, 2009L),
  Series  = "Council forecast (Total for all schools)"
)

city_all <- dplyr::bind_rows(city_actual, city_proj, city_council)

# Summary table for the five projection years (2027-2031: the window where
# we have both our projection and the council's forecast).
...
knitr::kable(
  ...
  align  = "lrrrr",
  format.args = list(big.mark = ","),
  caption = "City-wide Y7 projection (2027-2031): ours vs council's Appendix 6 total-for-all-schools"
)
```
...
p_city <- ggplot(city_all,
  ...
  scale_colour_manual(values = c(
    ...
    "Our projection (cohort-aged)"             = "#33a02c",
    "Council forecast (Total for all schools)" = "#e31a1c"
  )) +
  scale_x_continuous(breaks = seq(min(city_all$Year), max(city_all$Year), 1)) +
  labs(title = "City-wide Y7 offers: actuals, our projection, and council forecast",
       subtitle = "Sum across all 9 secondary schools (6 catchments + Cardinal Newman + Kings)",
...
At the city level the **overall direction is the same on both methods** — a steady decline in aggregate Y7 demand from ~2,300 in the mid-2020s to ~2,000 by 2031, a drop of roughly **10 %** over six years. That's consistent with the shrinking primary cohorts on our side, and the council's census-based school-roll decline on theirs.

The two methods agree most closely at the **ends** of the forecast window:

- **2027**: Ours **2,206**, Council **2,234** — Δ −28 (−1.3 %).
- **2031**: Ours **2,007**, Council **2,009** — Δ −2 (−0.1 %).

The biggest gap opens up in the middle years:

- **2028**: Ours **2,124**, Council **2,206** — Δ −82 (−3.7 %).
- **2029**: Ours **2,024**, Council **2,117** — Δ −93 (−4.4 %).
- **2030**: Ours **1,929**, Council **2,028** — Δ −99 (−4.9 %).

In every year our city-level projection sits **at or below** the council's. Our method finds that historical retention ratios (primary → secondary 7 years later) are low enough — especially at Longhill, BACA and PACA — that the city total lands around **100 pupils below** the council's demand forecast in 2028-2030. Hove Park / Blatchington Mill is the only catchment where our projection runs above the council's, and by not enough to cancel out the Longhill / BACA / PACA undershoot.

The practical implication: both methods project a substantial city-wide **surplus** of places by 2030–2031. The council sees aggregate demand of ~2,009 against total PAN of **2,500** — a surplus of **491 places (~20 %)**. Our method projects ~2,007 offers in 2031 — essentially the same aggregate — but gets there mostly through lower Longhill, BACA and PACA throughputs than the council expects, partly offset by higher Hove Park / Blatchington Mill throughput. So the two methods agree on the overall surplus scale but disagree on where in the city it will fall hardest.
...
Looking at the catchment-by-catchment numbers in the table above, three features stand out as **systematic rather than noise**:

1. **The direction of the gap at Longhill is one-sided every single year.** Our projection is **39 %–43 % below** the council's in all five forecast years (2027: 107 vs 166; 2028: 112 vs 176; 2029: 104 vs 159; 2030: 96 vs 151; 2031: 98 vs 154). That's not random fluctuation — it's the two methods reaching fundamentally different answers about what will happen at Longhill.
2. **BACA and PACA are consistently lower on our side.** Ours is below the council's in 10 of 10 BACA + PACA year-cells, with gaps of 10–30 %.
3. **Hove Park / Blatchington Mill is consistently higher on our side.** Ours is above the council's in 5 of 5 years, with gaps of 6–21 %.

The SDs in the retention-ratio table (0.02–0.23) give another read: if the ratios were themselves noisy, we'd expect a roughly symmetric mixture of "ours is higher" and "ours is lower" across catchments and years. What we see instead is a clear pattern per catchment, pointing to structural drivers.

### Why Longhill is the big one

Longhill is also the catchment where the council's **2025 and 2026 actuals undershot the forecast most dramatically** — 97 offers in 2025 against a forecast of 168, and 81 offers in 2026 against a forecast of 175. So the disagreement between our projection and the council's for 2027 onwards isn't theoretical: for the two years where both forecasts *can* be checked against actuals, **the council's method was substantially over-estimating Longhill demand and our method would have been much closer**.

The most likely structural reason: the council's method works by taking the current Oct-24 school census for each year group (children now in Longhill's primary catchment and currently in R–Y6) and applying a **single leakage percentage of 22.61 %**, then subtracting a fixed 55 + 3 = 58 going to CN/Kings. That 22.61 % leakage figure is already the highest in the city — so the council isn't blind to Longhill's difficulties — but it's still *much lower than what the actuals imply*. Backing out the implied leakage from the 2025/26 actuals gives ~65–70 %, i.e. roughly three times the council's assumed rate.

Put another way: the council's method carries a built-in assumption that the relationship between "children of secondary age resident in Longhill's catchment" and "Y7 offers actually made at Longhill High" is broadly stable, with a fixed percentage moving elsewhere. Our empirical-ratio method learns that this relationship has been steadily deteriorating across the six cohorts we have data for, and the 2025/26 actuals confirm that. If the decline continues, both methods are biased upward, but the council's much more so.
...
- The 22.61 % Longhill leakage is already the highest by a wide margin, which suggests the council has tried to calibrate for the specific problem.
- A method that decomposes census → fixed leakage % → fixed CN/Kings annual number will, by construction, propagate whatever the recent calibrated leakage was forward. If leakage is rising year-on-year (as Longhill's 2021–2026 actuals suggest it has been), the fitted percentage will lag behind reality until the calibration is revisited. That's a structural property of the method, not a judgement call.

But the *effect* is the same either way. For planning purposes, decisions about Longhill's future — whether to reduce PAN, consolidate provision, adjust catchment boundaries — will be taken on the basis of forecast demand figures that **both our method and the 2025/26 actuals suggest are optimistic by 50–100 pupils per year**. Whether one calls this "hope" or "methodological lag" is partly a matter of framing, but the practical implication is that if the council's Appendix 6 surplus numbers feel uncomfortable already, the underlying surplus at Longhill is very likely larger still.
...
**Summary**: the gaps are methodological, not noise. The direction of each gap is consistent with the structural features of each method — the council's fixed-leakage, census-driven approach tends to over-estimate demand in catchments where state-sector leakage is rising faster than the calibration assumes; our empirical-ratio approach picks that up automatically but is itself vulnerable if the future behaves very differently from the 2014–2019 cohorts. For Longhill specifically, the 2025 and 2026 actuals clearly back our projection over the council's, suggesting the council's forecast is optimistic there — not through hope, but through a method that struggles to keep up with an accelerating secular trend.
...
# the hidden `schools-catchments` setup chunk before the City-Wide section,
# since they are shared by the by-catchment cohort chart and these maps.
